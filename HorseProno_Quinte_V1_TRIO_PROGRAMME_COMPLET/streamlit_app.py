...
TRIO_MODEL_PATH = BASE_DIR / "trio_v1_artifact.json"
...
st.caption(
    ...
    "Choisis simplement la réunion puis la course à analyser — aucun CSV n'est nécessaire."
)
...
def run_prediction(history: pd.DataFrame, race: pd.DataFrame) -> pd.DataFrame:
    ...
    trio_artifact = load_model(str(TRIO_MODEL_PATH))
    ...
    result = race.copy().reset_index(drop=True)
    result["quinte_v1_score"] = predict_scores(artifact, features)

    # Rang Quinté V1 calculé sans modifier l'ordre original nécessaire aux features Trio.
    rank_frame = result[["horse_number", "quinte_v1_score"]].copy()
    rank_frame["_idx"] = np.arange(len(rank_frame))
    rank_frame = rank_frame.sort_values(
        ["quinte_v1_score", "horse_number"], ascending=[False, True]
    )
    rank_frame["quinte_rank"] = np.arange(1, len(rank_frame) + 1)
    rank_frame = rank_frame.sort_values("_idx")
    result["quinte_rank"] = rank_frame["quinte_rank"].to_numpy()

    # Trio V1 : il réutilise les variables pré-course de Quinté V1 + son score/rang.
    trio_features = features.copy()
    trio_features["v1_score"] = result["quinte_v1_score"].to_numpy()
    denom = max(1, len(result) - 1)
    trio_features["v1_rank_pct"] = (result["quinte_rank"].to_numpy() - 1) / denom

    missing_trio = [c for c in trio_artifact["features"] if c not in trio_features.columns]
    if missing_trio:
        raise RuntimeError("Variables Trio absentes : " + ", ".join(missing_trio))

    result["trio_v1_score"] = predict_scores(trio_artifact, trio_features)
    result = result.sort_values(
        ["quinte_v1_score", "horse_number"], ascending=[False, True]
    ).reset_index(drop=True)
    result["quinte_rank"] = np.arange(1, len(result) + 1)

    # Rang Trio uniquement à l'intérieur du Top 7 V1 : le pool Quinté V1 reste figé.
    result["trio_rank_in_top7"] = np.nan
    top7_idx = result.index[: min(7, len(result))]
    trio_order = result.loc[top7_idx].sort_values(
        ["trio_v1_score", "quinte_v1_score", "horse_number"],
        ascending=[False, False, True],
    ).index
    for rank, idx in enumerate(trio_order, start=1):
        result.loc[idx, "trio_rank_in_top7"] = rank
    return result


def build_trio_combinations(ranked: pd.DataFrame, n: int = 10) -> pd.DataFrame:
    pool = ranked.head(7).copy()
    if len(pool) < 3:
        return pd.DataFrame()

    rows = []
    eps = 1e-12
    for combo in itertools.combinations(pool.index.tolist(), 3):
        part = pool.loc[list(combo)].copy()
        # Produit en espace log pour la stabilité ; score affiché = moyenne géométrique.
        logs = [math.log(max(eps, float(v))) for v in part["trio_v1_score"]]
        combo_index = math.exp(sum(logs) / 3.0)
        ordered = part.sort_values(
            ["trio_v1_score", "quinte_v1_score", "horse_number"],
            ascending=[False, False, True],
        )
        rows.append({
            "Trio": " - ".join(ordered["horse_number"].astype(int).astype(str)),
            "Indice Trio": combo_index,
            "Rangs V1": " - ".join(sorted(part["quinte_rank"].astype(int).astype(str), key=int)),
            "Chevaux": " / ".join(ordered["horse_name"].astype(str)),
        })

    out = pd.DataFrame(rows).sort_values(
        ["Indice Trio", "Trio"], ascending=[False, True]
    ).reset_index(drop=True)
    out.insert(0, "Rang", np.arange(1, len(out) + 1))
    out["Indice Trio"] = out["Indice Trio"].round(4)
...
try:
    ...
    if is_quinte:
        ...
        st.warning(
            "Le modèle Quinté V1 a été calibré sur des supports Quinté officiels. "
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        st.subheader("🎲 Trio V1 — re-classement du Top 7")
        trio_top7 = top7.sort_values(
            ["trio_v1_score", "quinte_v1_score", "horse_number"],
            ascending=[False, False, True],
        )
        st.markdown(
            "### " + " - ".join(trio_top7["horse_number"].astype(int).astype(str).tolist())
        )
        st.caption(
            "Le pool reste exactement le Top 7 de Quinté V1. Trio V1 ne rajoute ni ne retire "
            "aucun cheval : il apprend seulement à les reclasser pour le podium."
        )

        trio_count = st.slider(
            "Nombre de combinaisons Trio à afficher",
            min_value=5, max_value=20, value=10, step=1,
        )
        trio_combos = build_trio_combinations(ranked, trio_count)
        if not trio_combos.empty:
            st.markdown(
                f"**Trio prioritaire : {trio_combos.iloc[0]['Trio']}**"
            )
            st.dataframe(trio_combos, use_container_width=True, hide_index=True)
            st.caption(
                "Indice Trio = score de classement, pas une probabilité calibrée. "
                "Le rétro-test final était trop petit pour conclure à un gain de rentabilité."
            )

        display = ranked[
            [
                "quinte_rank", "horse_number", "horse_name", "quinte_v1_score",
                "trio_v1_score", "trio_rank_in_top7",
                "odds", "draw", "weight", "age", "jockey", "trainer",
            ]
        ].copy()
        display["quinte_v1_score"] = display["quinte_v1_score"].round(4)
        display["trio_v1_score"] = display["trio_v1_score"].round(4)
        display = display.rename(
            columns={
                ...
                "trio_v1_score": "Score Trio",
                "trio_rank_in_top7": "Rang Trio (Top7)",
                "odds": "Cote PMU",
                "draw": "Corde",
                "weight": "Poids",
                "age": "Âge",
        ...
        st.download_button(
            ...
            file_name=(
                f"pronostic_quinte_v1_{selected_day.isoformat()}_R{reunion}C{course}.csv"
            ),
            mime="text/csv",
        )
            ...
            st.json(
                {
                    ...
                    "trio_model": trio_artifact.get("model_name"),
                    "trio_status": trio_artifact.get("status"),
                    "trio_training_supports": trio_artifact.get("training_supports"),
                    "trio_C": trio_artifact.get("C"),
                }
            )
