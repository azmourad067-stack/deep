...
q30, exp30 = generate_quinte_30(v51)
assert len(q30) == 30, len(q30)
assert q30['Combinaison'].nunique() == 30
...
for _, rr in q30.iterrows():
    vals = {int(rr[c]) for c in ['C1','C2','C3','C4','C5']}
    assert len(vals) == 5
    assert vals <= pool10
    assert len(vals & top5q) >= 2
    assert len(vals - top5q) >= 1
print('QUINTE30_POOL', v51.sort_values('quinte_rank').head(10).horse_number.astype(int).tolist())
print(exp30[['Rang','N°','Présence /30']].to_string(index=False))
