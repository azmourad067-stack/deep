...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        # Quinté : 50 tickets uniques de 5 chevaux construits uniquement avec le Top10 officiel.
        st.subheader("🏇 50 combinaisons Quinté — pool des 10 premiers")
        q50, q50_exposure = generate_quinte_50(ranked)
        if len(q50) == 50:
            ...
            st.caption(
                "50 combinaisons uniques de 5 chevaux. Le générateur favorise les rangs 1–5, "
            ...
            st.dataframe(q50, use_container_width=True, hide_index=True)
            with st.expander("Voir la fréquence d'utilisation des 10 chevaux"):
                exposure_display = q50_exposure.copy()
            ...
            st.code("\n".join(f"{int(r['Ticket']):02d}. {r['Combinaison']}" for _, r in q50.iterrows()), language=None)
        else:
            st.warning("Il faut au moins 10 partants classés pour générer les 50 combinaisons Quinté à partir du Top10.")
