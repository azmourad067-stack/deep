...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        # Quinté : 70 tickets intelligents de 5 chevaux, pool strict du Top10 officiel.
        st.subheader("🏇 70 combinaisons Quinté intelligentes — pool des 10 premiers")
        q70, q70_exposure, q70_scenarios = generate_quinte_70(ranked)
        if len(q70) == 70:
            top10_nums = ranked.sort_values("quinte_rank").head(10)["horse_number"].astype(int).tolist()
            st.success("Pool Top10 : **" + " - ".join(map(str, top10_nums)) + "**")
            st.caption(
                "Les 70 tickets sont construits en 4 scénarios : 1 noyau pur 5+0, 25 noyaux forts 4+1, "
                "34 scénarios équilibrés 3+2 et 10 scénarios ouverts 2+3. La sélection exploite le rang officiel, "
                "le consensus marché/fondamental, les scores V5.1 et la diversité des paires/triples. "
                "Le pool reste strictement limité aux 10 premiers."
            )
            ...
            st.warning("Il faut au moins 10 partants classés pour générer les 70 combinaisons Quinté intelligentes à partir du Top10.")
