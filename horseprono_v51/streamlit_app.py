...
from quinte30_v51 import generate_quinte_30
...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        # Quinté : 30 tickets uniques de 5 chevaux construits uniquement avec le Top10 officiel.
        st.subheader("🏇 30 combinaisons Quinté — pool des 10 premiers")
        q30, q30_exposure = generate_quinte_30(ranked)
        if len(q30) == 30:
            ...
            st.caption(
                "30 combinaisons uniques de 5 chevaux. Le générateur favorise les rangs 1–5, "
                "mais impose une rotation des rangs 6–10 et pénalise les tickets trop similaires. "
                "Chaque ticket contient au moins 2 chevaux du Top5 et au moins 1 cheval des rangs 6–10."
            )
            st.dataframe(q30, use_container_width=True, hide_index=True)
            with st.expander("Voir la fréquence d'utilisation des 10 chevaux"):
                exposure_display = q30_exposure.copy()
            ...
            st.code("\n".join(f"{int(r['Ticket']):02d}. {r['Combinaison']}" for _, r in q30.iterrows()), language=None)
        else:
            st.warning("Il faut au moins 10 partants classés pour générer les 30 combinaisons Quinté à partir du Top10.")
