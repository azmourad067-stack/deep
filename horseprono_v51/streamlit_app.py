...
from quinte_v51 import add_v51_meta_consensus, load_v51_artifact, V51_CONFIG
...
APP_NAME = "HorseProno Quinté V5.1 — Meta-Consensus"
PMU_BASE_URL = "https://online.turfinfo.api.pmu.fr/rest/client/1"
REQUEST_TIMEOUT = 25
HEADERS = {
    "User-Agent": "HorsePronoQuinteV51/5.1 (+https://streamlit.io)",
...
V51_META_ARTIFACT_PATH = BASE_DIR / "v51_meta_artifact.json"
...
st.title("🏇 HorseProno Quinté V5.1 — Meta-Consensus")
st.caption(
    "Le programme PMU complet de la journée est chargé automatiquement. "
    "V5.1 mesure le consensus et la divergence entre le moteur marché V4.5 et le moteur fondamental 0 % cotes V4.6. "
    "La sélection officielle reste V4.5/V4.6 : le méta-modèle fonctionne en shadow mode tant qu'il n'a pas validé un gain forward."
)
...
@st.cache_data(show_spinner=False)
def load_v51_meta(path: str) -> dict:
    return load_v51_artifact(path)
...
def run_prediction(
    ...
    meta_artifact = load_v51_meta(str(V51_META_ARTIFACT_PATH))
    ...
    v46 = add_v46_fundamental(history, race, v45, fund_artifact, fund_ranker)
    return add_v51_meta_consensus(v46, meta_artifact)
...
def save_v51_shadow_snapshot(
    ...
    """Persiste la lecture V5.1 shadow avec la clé serveur uniquement."""
    url = _secret("SUPABASE_URL")
    service_key = _secret("SUPABASE_SECRET_KEY") or _secret("SUPABASE_SERVICE_KEY")
    if not url or not service_key:
        return 0, "V5.1 shadow non enregistré : clé serveur Supabase non configurée."
    if ranked.empty or "v51_meta_top5_score" not in ranked.columns:
        return 0, "V5.1 shadow non disponible pour cette course."
    try:
        official = ranked[ranked["selected_top7"].astype(bool)].sort_values("quinte_rank")["horse_number"].astype(int).tolist()
        fundamental = ranked.sort_values("v46_fundamental_rank").head(7)["horse_number"].astype(int).tolist()
        shadow = ranked[ranked["v51_shadow_top7"].astype(bool)].sort_values("v51_shadow_rank")["horse_number"].astype(int).tolist()
        shadow3 = ranked[ranked["v51_shadow_top3"].astype(bool)].sort_values("v51_shadow_rank")["horse_number"].astype(int).tolist()
        swap_in_txt = str(ranked.iloc[0].get("v51_shadow_swap_in", "") or "")
        swap_out_txt = str(ranked.iloc[0].get("v51_shadow_swap_out", "") or "")
        def _parse_nums(txt: str) -> list[int]:
            return [int(x) for x in txt.split(",") if x.strip().isdigit()]
        runner_scores = {}
        for row in ranked.itertuples(index=False):
            runner_scores[str(int(row.horse_number))] = {
                "meta_top5": _safe_float(getattr(row, "v51_meta_top5_score", None)),
                "meta_winner": _safe_float(getattr(row, "v51_meta_winner_score", None)),
                "shadow_rank": _safe_int(getattr(row, "v51_shadow_rank", None)),
                "fund_rank": _safe_int(getattr(row, "v46_fundamental_rank", None)),
                "official_rank": _safe_int(getattr(row, "quinte_rank", None)),
                "role": str(getattr(row, "v51_meta_role", "") or ""),
            }
        row = {
            "race_date": day_iso,
            "meeting_number": int(reunion),
            "race_number": int(course),
            "hippodrome": hippodrome,
            "discipline": str(ranked.iloc[0].get("discipline", "")),
            "scheduled_start": scheduled_start.isoformat() if scheduled_start else None,
            "minutes_to_start": minutes_to_start,
            "official_top7": official,
            "fundamental_top7": fundamental,
            "shadow_top7": shadow,
            "shadow_top3": shadow3,
            "consensus_count": int(float(ranked.iloc[0].get("v51_consensus_count", 0) or 0)),
            "union_count": int(float(ranked.iloc[0].get("v51_union_count", 0) or 0)),
            "consensus_confidence": _safe_float(ranked.iloc[0].get("v51_consensus_confidence")),
            "divergence_index": _safe_float(ranked.iloc[0].get("v51_divergence_index")),
            "stance": str(ranked.iloc[0].get("v51_stance", "") or ""),
            "swap_in": _parse_nums(swap_in_txt),
            "swap_out": _parse_nums(swap_out_txt),
            "runner_scores": runner_scores,
            "model_name": "HorseProno_V5_1_MetaConsensus",
            "status": "pending",
        }
        endpoint = f"{url.rstrip('/')}/rest/v1/v51_meta_shadow_predictions"
        headers = _supabase_headers(service_key)
        headers["Prefer"] = "return=representation"
        response = requests.post(endpoint, headers=headers, json=[row], timeout=25)
        response.raise_for_status()
        return 1, "V5.1 shadow enregistré pour validation forward."
    except Exception as exc:
        return 0, f"V5.1 shadow non enregistré : {exc}"
...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        meta_snapshot_count, meta_snapshot_message = (0, "")
        if is_quinte:
            ...
            meta_snapshot_count, meta_snapshot_message = save_v51_shadow_snapshot(
                ranked, selected_day.isoformat(), reunion, course, hippodrome,
                scheduled_start=scheduled_start, minutes_to_start=minutes_before
            )
        ...
        st.subheader("🎯 Sélection officielle V4.5/V4.6 + Meta-Consensus V5.1")
        ...
        # V5.1 : méta-consensus shadow — aucune modification du Top7 officiel.
        if "v51_meta_top5_score" in ranked.columns:
            st.subheader("🧠 V5.1 — Meta-Consensus (shadow mode)")
            conf = float(pd.to_numeric(ranked.iloc[0].get("v51_consensus_confidence"), errors="coerce"))
            div = float(pd.to_numeric(ranked.iloc[0].get("v51_divergence_index"), errors="coerce"))
            stance = str(ranked.iloc[0].get("v51_stance", "CONSENSUS_MODERE"))
            union_count = int(float(ranked.iloc[0].get("v51_union_count", 0) or 0))
            consensus_count = int(float(ranked.iloc[0].get("v51_consensus_count", 0) or 0))
            shadow7 = ranked[ranked["v51_shadow_top7"].astype(bool)].sort_values("v51_shadow_rank")
            shadow3 = shadow7[shadow7["v51_shadow_top3"].astype(bool)].sort_values("v51_shadow_rank")
            e1, e2, e3, e4 = st.columns(4)
            e1.metric("Consensus", f"{consensus_count}/7")
            e2.metric("Confiance méta", f"{100*conf:.0f} %")
            e3.metric("Divergence", f"{100*div:.0f} %")
            e4.metric("Pool union", f"{union_count} chevaux")
            stance_txt = {
                "CONSENSUS_FORT": "🤝 Consensus fort",
                "MARCHE_META_PLUS_FORT": "📈 Marché plus convaincant",
                "FONDAMENTAL_META_A_SURVEILLER": "🧬 Fondamental à surveiller",
                "DIVERGENCE_ELEVEE": "⚠️ Divergence élevée",
                "CONSENSUS_MODERE": "🧠 Consensus modéré",
            }.get(stance, stance)
            st.info(f"Lecture V5.1 : **{stance_txt}**")
            st.markdown("**Top7 alternatif V5.1 shadow :** " + _nums(shadow7))
            st.markdown("**Winner Top3 V5.1 shadow :** " + _nums(shadow3))
            swap_in = str(ranked.iloc[0].get("v51_shadow_swap_in", "") or "")
            swap_out = str(ranked.iloc[0].get("v51_shadow_swap_out", "") or "")
            if swap_in or swap_out:
                st.caption(f"Alternative shadow : entrée(s) [{swap_in or '—'}] · sortie(s) [{swap_out or '—'}]. Aucune modification n'est appliquée au Top7 officiel.")
            else:
                st.caption("Le Top7 shadow confirme exactement les 7 chevaux officiels.")

        # e-Trio : 10 tickets, toutes les paires possibles parmi les 5 premiers officiels.
        st.subheader("🎟️ 10 combinaisons e-Trio — 2 bases dans le Top 5")
        etrio, etrio_pool = generate_etrio_10(ranked)
        if len(etrio) == 10:
            base_nums = ranked.sort_values("quinte_rank").head(5)["horse_number"].astype(int).tolist()
            assoc_nums = etrio_pool["horse_number"].astype(int).tolist() if not etrio_pool.empty else []
            st.success(
                "Bases potentielles Top5 : **" + " - ".join(map(str, base_nums)) + "**  ·  "
                "Associés retenus : **" + (" - ".join(map(str, assoc_nums)) if assoc_nums else "—") + "**"
            )
            st.caption(
                "Les 10 paires possibles parmi les 5 premiers sont utilisées une fois chacune. "
                "Le 3e cheval est choisi hors Top5 avec priorité au consensus V5.1/V4.6, au Top7 officiel, "
                "au Top7 shadow et à la réserve fondamentale. Les associés sont répartis pour diversifier les tickets."
            )
            st.dataframe(etrio, use_container_width=True, hide_index=True)
            st.code("\n".join(f"{int(r['Ticket']):02d}. {r['Combinaison']}" for _, r in etrio.iterrows()), language=None)
        else:
            st.warning("Pas assez de partants exploitables pour générer les 10 combinaisons e-Trio selon la règle 2 bases Top5.")

        # Quinté : 70 tickets rétro-calibrés de 5 chevaux, pool intelligent de 10.
        st.subheader("🏇 70 combinaisons Quinté rétro-calibrées — pool intelligent de 10 chevaux")
        q70, q70_exposure, q70_scenarios = generate_quinte_70(ranked)
        if len(q70) == 70:
            top10_nums = q70_exposure.sort_values("Rang pool")["N°"].astype(int).tolist()
            st.success("Pool combinatoire : **" + " - ".join(map(str, top10_nums)) + "**")
            st.caption(
                "Les 70 tickets sont choisis parmi les 252 combinaisons possibles avec une pondération rétro-calibrée : 60 % signal du jour + 40 % profil historique. "
                "Le nombre de tickets 5+0 / 4+1 / 3+2 / 2+3 n'est plus fixé à l'avance : il dépend du score combiné. La sélection exploite le rang officiel, "
                "le consensus marché/fondamental, les scores V5.1 et les fréquences historiques des rangs. "
                "Le pool conserve les 7 premiers officiels puis ajoute les 3 meilleures réserves fondamentales."
            )
            if not q70_scenarios.empty:
                sc = q70_scenarios.copy()
                sc["Part"] = sc["Part"].map(lambda x: f"{100*float(x):.0f} %")
                st.dataframe(sc, use_container_width=True, hide_index=True)
            st.dataframe(q70, use_container_width=True, hide_index=True)
            with st.expander("Voir l'exposition intelligente des 10 chevaux"):
                exposure_display = q70_exposure.copy()
                if "Taux" in exposure_display.columns:
                    exposure_display["Taux"] = exposure_display["Taux"].map(lambda x: f"{100*float(x):.0f} %")
                st.dataframe(exposure_display, use_container_width=True, hide_index=True)
            st.code("\n".join(f"{int(r['Ticket']):02d}. [{r['Scénario']}] {r['Combinaison']}" for _, r in q70.iterrows()), language=None)
        else:
            st.warning("Il faut au moins 10 partants classés pour générer les 70 combinaisons Quinté rétro-calibrées à partir du pool de 10 chevaux.")

        st.caption(
            "V5.1 a été entraînée sur 30 courses walk-forward puis vérifiée sur 28 courses suivantes. "
            "Le méta-modèle libre n'a pas encore battu la baseline sur ce bloc : ses propositions restent donc strictement shadow pour accumuler une validation forward propre."
        )
        ...
        display = ranked[
            [
                ...
                "v51_meta_top5_score", "v51_meta_winner_score", "v51_shadow_rank", "v51_shadow_top7", "v51_meta_role",
        ...
        if "v51_meta_top5_score" in display.columns:
            display["v51_meta_top5_score"] = (100 * pd.to_numeric(display["v51_meta_top5_score"], errors="coerce")).round(1)
            display["v51_meta_winner_score"] = (100 * pd.to_numeric(display["v51_meta_winner_score"], errors="coerce")).round(1)
            display["v51_shadow_rank"] = pd.to_numeric(display["v51_shadow_rank"], errors="coerce").astype("Int64")
            display["v51_shadow_top7"] = display["v51_shadow_top7"].map({True: "🧠", False: ""})
        ...
        display = display.rename(
            columns={
                ...
                "v51_meta_top5_score": "Score Meta Top5 %",
                "v51_meta_winner_score": "Score Meta Winner %",
                "v51_shadow_rank": "Rang V5.1 shadow",
                "v51_shadow_top7": "V5.1",
                "v51_meta_role": "Rôle V5.1",
        ...
        if is_quinte and meta_snapshot_message:
            if meta_snapshot_count:
                st.caption("🧠 " + meta_snapshot_message)
            else:
                st.caption("ℹ️ " + meta_snapshot_message)
